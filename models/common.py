...
class Bottleneck_Dynamic(nn.Module):
    # Standard bottleneck
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Dynamic_conv2d(c1, c_, 1, 1)
        self.cv2 = Dynamic_conv2d(c_, c2, 3, 1, g=g)
        self.add = shortcut and c1 == c2
...
class DC3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Dynamic_conv2d(c1, c_, 1, 1)
        self.cv2 = Dynamic_conv2d(c1, c_, 1, 1)
        self.cv3 = Dynamic_conv2d(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(Bottleneck_Dynamic(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
...
class Dynamic_conv2d(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None,ratio=0.25, d=1, g=1, bias=True, K=4,temperature=34, init_weight=True):
        ...
        self.bias = bias
        self.K = K
        self.attention = attention2d(c1, ratio, K, temperature)

        self.weight = nn.Parameter(torch.randn(K, c2, c1//g, k, k), requires_grad=True)
        if bias:
    ...
    def forward(self, x):#将batch视作维度变量，进行组卷积，因为组卷积的权重是不同的，动态卷积的权重也是不同的
        ...
        # 动态卷积的权重的生成， 生成的是batch_size个卷积参数（每个参数不同）
        aggregate_weight = torch.mm(softmax_attention, weight).view(batch_size*self.c2, self.c1//self.g, self.k, self.k)
        if self.bias is not None:
            aggregate_bias = torch.mm(softmax_attention, self.bias).view(-1)
            output = F.conv2d(x, weight=aggregate_weight, bias=aggregate_bias, stride=self.s, padding=self.p,
                              dilation=self.d, groups=self.g*batch_size)
        else:
            output = F.conv2d(x, weight=aggregate_weight, bias=None, stride=self.s, padding=self.p,
                              dilation=self.d, groups=self.g * batch_size)

        output = output.view(batch_size, self.c2, output.size(-2), output.size(-1))
        return output
